...
1) If a cell of any kind (excluding zombies) has less then 2 neighbours the cell dies from underpopulation.
...
5) If the number of preditor neighbours is greater then the number prey neighbours the prey die.
6) If a dead preditor or prey have 3 neighbours of the same kind and no neighbours of other kind the cell becomes alive.

The program initalizes with a random number of prey cell and no other kind.
To insert zombies press the pause button and right click where you would like the zombie.
To insert preditors press the pause button and press your up arrow key where you would like the preditor.
